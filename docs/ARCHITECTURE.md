...
4. At poll time, one question record is appended per unique question to
   `questions.jsonl` inside the meeting's directory, sitting next to
   that meeting's `events.parquet`. Each record carries a UUIDv4
   `question_id` plus the full question content (prompt, type, choices,
   correct answer). `challenge_issued` events in the Parquet reference
   that UUID. The `--type` label travels with the event row, not with
   the `.jsonl` record.
...
7. All events are written to
   `<meetings_dir>/<rendered-template-name>/events.parquet` by
   `eventstore`; the meeting directory itself is the unit of identity.
8. For CSV report generation, Go invokes
   `ptrack_py report <meeting-dir>` and reads the CSV off stdout.
...
- `POST /system/shutdown` — stops the active session, drains the
  in-process challenger, closes all listeners, and returns 200 once
...
```
ptrack_py report <meeting-dir> [<dir2> …]
ptrack_py stats  <meeting-dir> [<dir2> …]
```

Both commands take positional per-meeting directory paths or glob
patterns and write their result (CSV or JSON) to stdout. `report`
produces a per-meeting CSV when exactly one directory matches and the
cross-meeting aggregate when more do.

Go reads the result off stdout directly and renders it on the fly —
neither the CSV nor the stats JSON is cached on disk. Exit code +
...
```
Browser (getUserMedia + MediaRecorder) ──HTTP POST /audio/segment──► Go control plane ──HTTP──► ASR backend
   ▲                                                                          │
...
- a working path on mobile, including Android-via-Termux: the browser
  on the phone captures audio and POSTs each batched segment to the
  daemon over loopback HTTP;
- a mute control that lives where the teacher is already looking, with
  no audio captured before they explicitly start.

The audio bytes are never written to disk by Go or the browser. The
browser uses `MediaRecorder` to batch microphone input into short
Opus/WebM segments and uploads each segment as a regular
`POST /audio/segment` request — no WebSocket, no streaming framing.
`internal/challenger/` forwards the segment to the ASR endpoint as it
arrives, and the returned transcripts accumulate into a rolling
window of `transcript_window_minutes` minutes; older content is
evicted in memory, never flushed to disk.
...
stdout-as-result is debuggable from a shell
(`ptrack_py report <meeting-dir>`), has no IPC state to maintain, and
recycles all memory between runs. Go reads the output and renders it
on the fly; no on-disk cache.
...
├── meetings/                           # meetings_dir; one subdir per meeting
│   ├── 2026-04-21-algebra/
│   │   ├── events.parquet
│   │   └── questions.jsonl
│   └── 2026-04-23-algebra/
│       ├── events.parquet
│       └── questions.jsonl
└── generated.yaml                      # challenges.auto_generation.review_dir/bank_basename.yaml; overwritten in place
```
...
├── meetings\                           # meetings_dir; one subdir per meeting (events.parquet + questions.jsonl)
├── generated.yaml                      # challenges.auto_generation.review_dir\bank_basename.yaml; overwritten in place
